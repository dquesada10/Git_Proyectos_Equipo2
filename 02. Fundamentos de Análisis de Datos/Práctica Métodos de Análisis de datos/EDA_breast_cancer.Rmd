...
Lo primero importamos las librerias que vamos a utilizar en el informe de markdown así como la version de R que estamos utilizando:
...
Vemos que en alguna algunas variables hay N/A pero son muy pocas. De hecho, suponen un 0.47% del total de los datos. Además, de estos registros no tenemos la variable respuesta (Status) con lo que optamos por eliminarlas.
...
Vamos a empezar haciendo un modelo de regresión lineal multiple incluyendo todas las variables para ver de un primer vistazo cuales son más significativas para predecir la variable Tumor_Size (nombre del modelo: modelo_inicial1):
...
Se puede observar que la variable T Stage es significativa ya que sus p-valores son < 5% por lo que se rechazaría la hipótesis que el coeficiente sea 0.

Esto también se observa con la variable 6th_Stage para el nivel IIIA así como para ciertos niveles del numero de nodos positivos.

El R² ajustado es de un 76% por lo que no está mal el modelo pero incluye muchisimas variables que no son significativas para predecir la variable respuesta y que añaden complejidad al modelo.

Es por esto que hacemos prueba de lanzar el modelo unicamente con las 3 variables que salieron muy significativas (nombre del modelo: modelo_inicial2):
...
Antes de realizar el procesado de las variables cualitativas, vamos a utilizar una de las técnicas automáticas de selección de variables, en particular vamos a utilizar el metodo forward para analizar las variables que tendria sentido incorporar: 
...
Las conclusiones son muy parecidas aunque ahora una variable como Regional_Node_Positive_R(3,10] la descarta para incluir y sin embargo considera meter algun nivel de la variable differentiate.


### Procesado de variable cualitativas

En base a las que vemos que son significativas, cuando son categóricas tenemos que volverlas numéricas, Lo hacemos para las que han salido significativas anteriormente.
...
En base a las variables seleccionadas y despúes de haber procesado las variables cualitativas que vamos a incluir en el modelo para volverlas dummie variables (numéricas de 0 y 1), lanzamos el modelo para analizar resultados (nombre del modelo: modelo1):
...
Como observamos que las 3 últimas variables o no han entrado porque no tenían valores aunque con el método forward salia que se tenían que incluir o no es tan significativa como es el caso de differentiate para "Poorly differentiated" o por ejemplo el rango de nodos positivos ya que sale un p-valor de 8% con lo que no se rechaza la hipótesis nula (coeficiente de 0).
...
Observando los resudios del modelo se observa que el modelo no es muy bueno porque estos no se distribuyen de manera normal y en estos se observa un ligero grado de heterocedasticidad en el modelo ya que la dispersion de estos aumenta conforme aumentan los valores de la variable respuesta.

Antes de pasar a hacer un backtest sobre este modelo2, antes vamos a comparar los distintos modelos creados por el estadístico BIC:
...
Se observa que el modelo 2 es el mejor de los modelos pero por muy poco ya que tiene una capacidad de predecir similar pero utiliza menos parametros ya que el BIC penaliza los modelos muy complejos que pueden generar overfitting.
...
Por último, vamos a mirar que variables saldrian significativas si no utilizaramos las variables T_Stage, N_Stage o sixth_Stage. Al final estas variables están directamente ligadas al tamaño del tumor, sobre todo T_Stage y sixth Stage que a su vez engloba al concepto de T_Stage porque T_Stage toma niveles en función del tamaño del tumor. Es verdad que el resto de variables tambien son descriptivas del grado de cancer salvo la edad, la raza y el marital status y lo interesante sería disponer de más variables como estas últimas para tratar de predecir el tamaño del tumor, es decir, variables sobre el estilo de vida de la persona (fuma/no fuma), sobre otras caracteristicas de la persona (peso) o geograficas (codigo postal, ciudad/campo...) por poner varios ejemplos.
...
Por último vamos a comparar los resultados por estos grupos para ver si el tumor medio real del predecido es muy diferente.
...
Ahora vamos a hacer cross validation, que basicamente significa que se particiona el dataset de train en k partes y para parte se divide un un subset de train y de test y se lanza el modelo para las k partes y luego se promedian los resultados. Para ver que nuestro modelo seleccionado es robusto (modelo2), nuestros coeficientes resultantes del cross validation deberían ser parecidos.
...
* La muestra recoge mujeres pacientes de cancer de mama que en general están entre los 50 y 60 años, son blancas, están casada, han sobrevivido en torno a 6 años y con un nivel de cáncer de mama moderados según los distintos paramentros del nivel de cáncer del dataset. De ahí que al final del estudio un 85% haya sobrevivido.
* Únicamente habia 19 datos faltantes que se han eliminado porque no suponían más que el 0.47% de la muestra y no incluian ni la variable respuesta que es si sobreviven o no.
* Respecto a las correlaciones entre las variables numéricas:
  + Se observa que los meses sobrevividos tiene una correlacion inversa lineal con si la mujer fallece.
