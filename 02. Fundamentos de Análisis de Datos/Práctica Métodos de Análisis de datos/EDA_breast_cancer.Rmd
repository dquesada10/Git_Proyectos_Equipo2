...
Como observamos que las 3 ultimas variables o no han entrado porque no tenian valores aunque con el metodo forward salia que se tenian que incluir o no es tan significativa como es el caso de differentiate para "Poorly differentiated".

Por tanto optamos por hacer un nuevo modelo en el que las eliminamos:
```{r}
model2<- lm(Tumor_Size ~ T_Stage2 + T_Stage3+T_Stage4+N_Stage2+sixth_Stage_IIIA+sixth_Stage_IIIC, data = Breast_Cancer_train_mod)
summary(model2)
```
...
Observando los resudios del modelo se observa que el modelo no es muy bueno porque estos no se distribuyen de manera normal y en estos se observa un alto grado de heterocedasticidad en el modelo ya que la dispersion de estos aumenta conforme aumentan los valores de la variable respuesta.

### Backtest contra el dataset de test

## Conclusiones

La muestra es en rasgos fgenerales.

Tanto % sobrevive y tantos mueren
Las variasbles que mas explican la variable respuesta son
insights sobre los datos son
